...
from app.core.deps import get_current_user, get_redis, oauth2_scheme, get_authenticated_user
from app.schemas.auth import LoginRequest, Token, UserResponse, UserCreate, RefreshRequest, PasswordChangeRequest
...
def login_access_token(
    ...
    if not user:
        ...
        log_activity(db, "auth.login_failed", user=user,
                     details={"attempted_login": form_data.username, "reason": "inactive_account"},
                     request=request)
        db.commit()
...
def logout(
    ...
    log_activity(db, "auth.logout", user=current_user, resource="user",
...
def change_password(
    ...
    db.add(current_user)
    log_activity(db, "auth.password_changed", user=current_user, resource="user",
                 resource_id=current_user.id)
    db.commit()
