...
from app.api import (
    partners, quotes, imports, matching, customers,
    generated_quotes, cities, customer_quotes, auth, users,
    activity_logs, stats, alerts,
)
...
# Audit & Statistiques (ADMIN / SUPER_ADMIN uniquement)
api_router.include_router(activity_logs.router,    prefix="/activity-logs",    tags=["Historique Activités"])
api_router.include_router(stats.router,            prefix="/stats",            tags=["Statistiques"])
api_router.include_router(alerts.router,           prefix="/alerts",           tags=["Alertes Sécurité"])
